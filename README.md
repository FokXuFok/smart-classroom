...
特色能力：**SSE 实时签到大屏**（学生签到即时推送教师看板，无需刷新）、**ECharts 可视化**（签到趋势/互动统计/预警学生出勤率图表）、**Excel 一键导出**（单次考勤表 + 作业成绩册 .xlsx）、**AI 助教**（Vue3 教师端：AI 备课助手、答疑热词、作业错误分析 Agent、作业 AI 批量批改与反馈提前开放、知识库/评分规则管理）。

教学组织模型贴合**大学合班授课**（桂林信息科技学院）：同一门课常由多个行政班合班上课，教师端课程、考勤等一律以**课程**为聚合单位——"我的课程 · 上课安排"一门课一行，合并展示该课全部选课人数、教室与上课时段，不做班级拆分/按班筛选（班级仅作为学生的行政归属保留）。
...
Vue3 登录页（`http://127.0.0.1:5173/login`）含**注册**入口：学生/教师自选身份注册（学生需选行政班），提交后待管理员审批通过方可登录（审批流见管理端）。
...
├── frontend/                # Vue3 + Element Plus 多端工程（教师/辅导员/管理员 + 登录注册，Vite 5173）
...
- **教学组织（大学合班）**："我的课程 · 上课安排"（`GET /api/teacher/courses/teaching`）以课程为一行聚合选课人数 + 教室 + 上课时段（取课表该课程多时段/多教室并集），适配桂林信息科技学院多个行政班合班的大班授课模式；签到/作业/互动均按课程开展，不做班级筛选。
- **AI**：阿里百炼 OpenAI 兼容接口（deepseek-v4-pro-0813），Key 从 `.env` 的 `DASHSCOPE_API_KEY` 读取。教师端集成：
  - **AI 助教页**（frontend TeacherAI.vue）：AI 备课助手（按主题生成教案）、答疑热词统计、作业错误分析 Agent、知识库 CRUD、评分规则 CRUD；
  - **作业流 AI**：作业列表"AI 整批批改"（对已评测提交批量生成 AI 反馈，AI 不可用自动降级规则反馈）、可提前向学生开放 AI 反馈；
  - **班级错误分析**（`POST /api/ai/homework/{id}/analyze-errors`）：聚合编译错误 / 未通过用例（剥离 expected 答案）生成"高频错误统计 + 讲评建议"。
